...
const createListItem = (index, option) => {
  const textBox = createDomElement(
    "div",
    {
      class: "textBox",
    },
  ...
  const textBoxWrapper = createDomElement(
    "div",
    {
  ...
  const thumbnail = createDomElement(
    "img",
    {
      class: "thumbnail",
      src: option.previewImage,
  ...
  const thumbnailBox = createDomElement(
    "div",
    {
      style: `width:${5 * scaleFactor}vh;height:${5 * scaleFactor}vh;`,
      class: "thumbnailBox",
    },
    [thumbnail]
  );
  const listItem = createDomElement(
    "div",
    {
      style: `width:${20 * scaleFactor}vh;height:${6 * scaleFactor}vh;`,
      class: `listItem ${index === selectedItem ? "selected" : ""}`,
    },
    [thumbnailBox, textBoxWrapper]
  );
  return listItem;
};
let factor = 1;
...
const fillText = (List, option, index) => {
  ...
  if (currChild.clientHeight <= 3 * maxHeight) {
    mid = "";
  }
  while (l <= r) {
  ...
  List.childNodes[index].childNodes[1].childNodes[0].innerHTML =
    prefix + mid + suffix;
};
...
let curr =
  List.childNodes[List.childNodes.length - 1].childNodes[1].childNodes[0];
